...
<div>
    ...
    <button @click="CreateNew ? CreateNew = !CreateNew :CreateNew = !CreateNew" class="btn btn-success form-group bn">
        New Product</button>
    ...
    <form class="d" v-if="CreateNew">
        <div class="form-row">
            <div class="col-md-12">
                ...
                <div class="fileUpload">
                    <input @change="onFileNewImg($event)" type="file" class="upload" enctype="multipart/form-data" />
            ...
            <div class="form-group col-md-6">
                <label>Name</label>
                <input v-model="NewProdData.name" type="text" class="form-control" placeholder="Product name">
            </div>
            <div class="form-group col-md-6">
                <label>Price</label>
                <input v-model="NewProdData.price" type="number" class="form-control" placeholder="Product Price">
            </div>
        </div>
        <div class="form-group">
            <label>Desc</label>
            <input v-model="NewProdData.desc" type="text" class="form-control" placeholder="Description">
        </div>
        ...
        <div class="form-row">
            ...
            <div class="form-group col-md-4">
                ...
                <select v-model="NewProdData.CatID" v-on:change="ChangeNewProdCat($event)" class="form-control">
                    <option v-for="Catdata in CatgeorisesList" :key="Catdata.id" :value="Catdata.id">
                        {{Catdata.name}}
                    </option>
                </select>
        ...
        <button @click="AddNewProduct($event)" type="submit" class="btn btn-primary">Add</button>
    </form>
    ...
    <div v-if="!CreateNew">
        ...
        <div v-for="data in NewProductsList" :key="data.id">
            ...
            <form class="d" v-if="data.isEdit">
                <div class="form-row">
                    <div class="col-md-12">
                        ...
                        <div class="fileUpload">
                            <input @change="onFileChange($event, data.id)" type="file" class="upload" enctype="multipart/form-data" />
                            <span>Upload</span>
                        </div>
                ...
                <div class="form-row">
                    ...
                    <div class="form-group col-md-4">
                        ...
                        <select v-on:change="ChangeProductCat($event, data.id)" class="form-control">
                            <option v-for="catdata in CatgeorisesList" :key="catdata.id" :selected="CheckTheCat(catdata.id, data.id)" :value="catdata.id">
                                {{catdata.name}}
                            </option>
                        </select>
                    </div>
            ...
            <div class="card d" v-if="!data.isEdit">
                ...
                <div class="card-body">
                    <a @click="data.isEdit = !data.isEdit" class="card-link btn btn-danger">Edit</a>
                </div>
            </div>
    ...
    <hr>
    <PageNationStore v-if="!CreateNew" v-on:new-ProductsData="NewProductData" ComponentName="ProductComponent" />
